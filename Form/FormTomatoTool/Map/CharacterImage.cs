...
namespace TomatoTool
{
		...
		Form
	{
		...
		private void numericUpDownCharacterImageSize_ValueChanged(object sender, EventArgs e)
		{
			pictureBoxCharacterImage.Refresh();
		}
		...
		private void pictureBoxCharacterImage_Paint(object sender, PaintEventArgs e)
		{
			if (listBoxCharacterImage.SelectedItem != null)
			{
				...
				{
					Bitmap b = characterImage.TileList[i].toBitmap(((Map)listBoxMap.SelectedItem).CharacterScriptList.Palette[trackBarCharacterImagePalette.Value]);
		...
		private void pictureBoxCharacterImageAction_Paint(object sender, PaintEventArgs e)
		{
			if (listBoxCharacterImage.SelectedItem != null)
			{
				...
				try
				{
					e.Graphics.DrawImage(characterImage.toBitmap(CharacterScriptDirection.Down, 0, ((Map)listBoxMap.SelectedItem).CharacterScriptList.Palette[trackBarCharacterImagePalette.Value], true), 0, 0);
				}
				catch
				{
				}
		...
		{
			CharacterImageEditor characterImageEditor = new CharacterImageEditor((CharacterImage)listBoxCharacterImage.SelectedItem, ((Map)listBoxMap.SelectedItem).CharacterScriptList.Palette[trackBarCharacterImagePalette.Value]);
