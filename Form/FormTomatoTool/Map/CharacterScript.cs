...
namespace TomatoTool
{
	public partial class FormTomatoTool
		...
		Form
	{
		private void initializeCharacterScript()
		{
			listBoxCharacterScript.DataSource = ((Map)listBoxMap.SelectedItem).CharacterScriptList.CharacterScript;
			...
			if (listBoxCharacterScript.Items != null)
			{
				switch (e.Button)
				{
					case MouseButtons.Left:
						foreach (CharacterScript characterScript in listBoxCharacterScript.Items)
						{
							if ((characterScript.X == (e.X / Map.BLOCK_WIDTH)) &&
								(characterScript.Y == (e.Y / Map.BLOCK_HEIGHT)))
							{
		...
		{
			if (((Map)listBoxMap.SelectedItem).CharacterScriptList.CharacterScript.Count == 0)
			{
